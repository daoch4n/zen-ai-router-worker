To optimize your AI code review GitHub Action for faster execution without hurting review quality, we can introduce parallelism within your Python script for processing files and also add dependency caching to your workflow.

**Key Optimization Strategies:**

1.  **Parallel File Processing in Python Script**: The most significant time is spent on API calls to Gemini for each file. We can process multiple files' prompts and handle their responses concurrently using Python's `concurrent.futures.ThreadPoolExecutor`.
    *   Prompt creation (which includes reading file content) for different files can happen in parallel.
    *   Actual API calls will be serialized by a lock to safely manage the global configuration of the `google-generativeai` client and respect rate limits. However, the overall process will still be faster as I/O operations (file reading, waiting for API) and CPU-bound tasks (prompt string manipulation, JSON processing) for different files can be overlapped.
2.  **Thread Safety**: Ensure that shared resources like the `GeminiKeyManager` and the Gemini client configuration are accessed in a thread-safe manner.
3.  **Workflow Dependency Caching**: Cache Python dependencies to speed up the `Set up Python and Install Dependencies` step on subsequent runs.
4.  **Configurable Parallelism**: Make the number of parallel workers configurable via an environment variable.

**Review Quality**: These changes are designed to maintain review quality. Each file is still analyzed with the same prompting logic and context. Parallelism affects the *order and timing* of processing, not the *method* of processing for each file.

Here are the suggested modifications:

**1. Python Script Modifications (`.github/workflows/gemini-pr-review.py`)**

Add necessary imports and locks, modify `GeminiKeyManager` for thread safety, create a task function for single file analysis, and update `analyze_code` to use `ThreadPoolExecutor`. Also, implement a lock for Gemini API interactions.
...
# --- GitHubAuthenticator class remains the same ---
# (Assuming GitHubAuthenticator is as provided in the input)
    ...
    def rotate_key(self):
        with self._lock:
            ...
            if self.fallback_key and "GEMINI_FALLBACK_API_KEY" not in self.rate_limited_keys:
                ...
                # If primary was rate limited and fallback is not available or also rate limited, we are stuck for now
                # but reset to primary to allow future retries if quota resets.
                if self.current_key_name != "GEMINI_API_KEY" or "GEMINI_API_KEY" in self.rate_limited_keys : # if we were on fallback or primary is already marked
                    self.current_key = self.primary_key
                    self.current_key_name = "GEMINI_API_KEY"
                    # If we reset to primary, and it was rate limited, this cycle might repeat.
                    # Clearing rate_limited_keys allows trying all keys again in the next cycle if all were exhausted.
                ...
                self.all_keys_rate_limited = True # Mark that we've hit limits on all known keys at some point
                # self.used_fallback_key should remain true if it was used, or false otherwise. Don't reset here.
                return False


    def is_rate_limit_error(self, error):
        # This method might be called frequently, lock only if modifying shared state
...
def initialize_gemini_client():
    ...
    # Initial configuration, will be re-configured per API call if keys rotate
...
# --- PRDetails, get_pr_details, get_diff, get_hunk_representation, get_file_content,
# --- load_previous_review_data, get_previous_file_comments, create_batch_prompt
# --- remain largely the same. Ensure logger is used instead of print where appropriate.

# (Copy these classes/functions from your original script, ensuring they use `logger`)
class PRDetails:
...
def get_diff(pr_details: PRDetails, comparison_sha: Optional[str] = None) -> str:
    # (Content from original script - ensure use of logger)
            ...
            if comparison_obj and comparison_obj.files:
                for file_diff in comparison_obj.files:
                    if file_diff.patch:
                        ...
                        # No specific header for 'modified' beyond diff --git and index from patch
    ...
    try:
        ...
        if diff_text:
            ...
            return "" # Important to return empty string, not None
    except GithubException as e:
    ...
    try:
        request_auth = GitHubAuthenticator() # Use the class
        _, token = request_auth.authenticate()
        if token:
            headers['Authorization'] = f'token {token}'
        else: # Fallback if app auth fails but GITHUB_TOKEN might be valid from environment
...
def get_hunk_representation(hunk: Hunk) -> str:
    return str(hunk) # unidiff Hunk object has a __str__ method
...
def get_file_content(file_path: str) -> str:
    ...
    try:
        ...
        if p_file_path.exists() and p_file_path.is_file():
            ...
            max_initial_read_bytes = 300000  # 300KB
            if file_stat.st_size > max_initial_read_bytes:
                ...
                # No end content for very large files to keep it simple and avoid large memory use
                full_file_content = start_content + "\n\n... [content truncated due to very large size] ...\n\n"
            else:
                with open(p_file_path, 'r', encoding='utf-8', errors='ignore') as f:
                    full_file_content = f.read()

            max_char_len_for_context = 150000 # 150k chars
...
def get_previous_file_comments(review_data: Dict[str, Any], file_path: str) -> List[Dict[str, Any]]:
    ...
    for comment in review_data.get("review_comments", []):
        ...
        if comment.get("file_path") == file_path and "[IGNORED]" not in comment_text: # Basic ignore check
...
GEMINI_RPM_LIMIT = int(os.environ.get("GEMINI_RPM_LIMIT", 45)) # Configurable RPM
...
def improved_calculate_github_position(file_patch: PatchedFile, hunk_idx: int, line_num_in_hunk: int) -> Optional[int]:
    ...
    try:
        ...
        hunk_lines = list(target_hunk) # Lines in the hunk including context, additions, deletions
        num_lines_in_hunk = len(hunk_lines)

        # line_num_in_hunk is 1-based index into the hunk's content lines
        # (lines after '@@ ... @@')
        ...
        position = 0
        for i in range(hunk_idx): # Sum lengths of previous hunks
            position += 1 # For hunk header '@@ ... @@'
            position += len(list(hunks_in_file[i]))
        position += 1 # For target hunk's header '@@ ... @@'
        position += line_num_in_hunk -1 # 0-indexed offset within the hunk content lines
...
def get_ai_response_with_structured_output(prompt: str, model_name: str, max_retries: int = 3) -> List[Dict[str, Any]]:
    global gemini_key_manager # Should be properly initialized
    ...
    for attempt in range(1, max_retries + 1):
        ...
        # The gemini_api_interaction_lock protects Client.configure and generate_content
        with gemini_api_interaction_lock:
            try:
                ...
                enforce_gemini_rate_limits() # This has its own lock, called while holding api_interaction_lock
                ...
                # Processing response (same as original script)
                ...
                if hasattr(response, 'candidates') and response.candidates and hasattr(response.candidates[0], 'content') and hasattr(response.candidates[0].content, 'parts'):
                    ...
                    if hasattr(part, 'function_call') and part.function_call and hasattr(part.function_call, 'args'): # Pyhton SDK uses 'args'
                        logger.info("Received structured output response with function_call.args.")
                        data_to_process = part.function_call.args # This should be a dict
                    elif hasattr(part, 'text'): # Fallback if not function_call but text (might be JSON string)
                         try:
                            ...
                            data_to_process = None # Will be handled below
                
                if data_to_process:
                    return process_structured_output(data_to_process)

                # Fallback for older response structures or direct JSON in text
                ...
                if hasattr(response, 'text') and response.text:
                    try:
                        ...
                        return []
                elif hasattr(response, 'parsed_message'): # some SDK versions might use this
                ...
                # Rate limit check and rotation logic (gemini_key_manager methods are thread-safe)
                if gemini_key_manager.is_rate_limit_error(e):
                    ...
                    if gemini_key_manager.rotate_key():
                        ...
                        # No 'continue' here, the lock will be released and re-acquired by the same thread for the next attempt in its loop
                    else:
                        logger.warning("All API keys are rate limited or unavailable. Will retry after delay if attempts remain.")
                        if attempt < max_retries: time.sleep(15 * attempt); # Fall through to next attempt
                        else: return [] # All retries failed
                
                if attempt < max_retries:
                    time.sleep(5 * attempt)
                    # Continue to the next attempt in the for loop (releases and re-acquires lock)
                else: # Max retries reached for this specific API call
                    return []
...
def _analyze_file_task(patched_file: PatchedFile, pr_details: PRDetails) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        ai_reviews_for_file = get_ai_response_with_retry(batch_prompt) # This now handles API call serialization internally
...
def analyze_code(files_to_review: Iterable[PatchedFile], pr_details: PRDetails) -> List[Dict[str, Any]]:
    ...
    try:
        max_workers_str = os.environ.get("MAX_PARALLEL_FILE_REVIEWS", "3")
        max_workers = int(max_workers_str)
        if max_workers <= 0:
            logger.warning(f"MAX_PARALLEL_FILE_REVIEWS was '{max_workers_str}', defaulting to 1 (sequential).")
            max_workers = 1
    except ValueError:
        logger.warning(f"Invalid MAX_PARALLEL_FILE_REVIEWS value, defaulting to 3.")
        max_workers = 3
    
    logger.info(f"Using up to {max_workers} parallel workers for file analysis.")

    if max_workers == 1: # Sequential execution if max_workers is 1
...
# --- get_hunk_header_str, process_batch_ai_reviews, save_review_results_to_json,
# --- detect_severity, detect_category, create_review_and_summary_comment,
# --- parse_diff_to_patchset, main
# --- remain largely the same. Ensure logger is used instead of print.
# (Copy these functions from your original script, ensuring they use `logger`)
def get_hunk_header_str(hunk: Hunk) -> str:
    # (Content from original script)
    ...
    for review_detail in ai_reviews:
        try:
            ...
            if github_pos_result is None:
                ...
                gh_comment_data["position"] = 1 # File-level comment as fallback
...
def save_review_results_to_json(pr_details: PRDetails, comments: List[Dict[str, Any]], filepath_str: str = "reviews/gemini-pr-review.json") -> str:
    ...
    rate_limited = False 
    if gemini_key_manager: # Check if initialized
        # This reflects the state *after* all API calls for the review
        if gemini_key_manager.used_fallback_key : # If fallback was used at any point
...
def detect_severity(comment_text: str) -> str:
    ...
    confidence = "medium"
    if "**my confidence: high**" in lower_text: confidence = "high" # Adjusted to match comment format
...
def create_review_and_summary_comment(pr_details: PRDetails, comments_for_gh_review: List[Dict[str, Any]], review_json_path: str):
    # (Content from original script - ensure use of logger and correct GitHub client handling)
    if not pr_details.pr_obj:
        logger.error("PR object not available in PRDetails. Cannot create review or comments.")
        return

    pr_for_comments = pr_details.pr_obj # Use the PR object from pr_details
    num_suggestions = len(comments_for_gh_review)
    
    # Attempt to use the bot-authenticated client if available and different from the initial one
    # The global 'gh' should be the bot-authenticated client if App Auth succeeded
    if gh and gh != pr_details.pr_obj.repository._requester.auth: # A bit of a heuristic check
        try:
            ...
            # pr_for_comments remains pr_details.pr_obj
    ...
    # Summary Comment (largely same, ensure pr_for_comments is used for issue comment)
    ...
    if gemini_key_manager:
        ...
        elif gemini_key_manager.encountered_rate_limiting: # Primary encountered limit, but maybe recovered or no fallback
            api_key_status_msg = "primary key (encountered rate-limiting)"
        
        if gemini_key_manager.all_keys_rate_limited and num_suggestions == 0 : # only warn if all keys hit limit AND no results
...
def parse_diff_to_patchset(diff_text: str) -> Optional[PatchSet]:
    ...
    try:
        ...
        logger.debug(f"Diff text that failed (first 1000 chars): {diff_text[:1000]}") # Use debug for potentially large output
    return None

def main():
    # (Content from original script - ensure use of logger)
    logger.info("Starting AI Code Review Script...")
    if not gh or not gemini_client_module: # gh is from global scope
    ...
    try:
        ...
        comparison_sha_for_diff = base_sha # Default
        ...
        if pr_details.event_type in ["opened", "reopened"]:
            ...
            else: # No last run, or last run is current head (e.g. re-run failed job)
                logger.info(f"Event 'synchronize', but no suitable last_run_sha. Reviewing full PR against base SHA: {base_sha}")
                comparison_sha_for_diff = base_sha
        else: # Other events (e.g. comment trigger - though current workflow doesn't use this)
        ...
        if not head_sha or not comparison_sha_for_diff:
            ...
            # Create empty review to satisfy workflow commit step
        ...
        if not initial_patch_set:
            ...
            save_review_results_to_json(pr_details, [], "reviews/gemini-pr-review.json") # Save empty
            # No need to raise, just exit gracefully for workflow
            return
        ...
        for pf_obj in initial_patch_set:
            # Normalize path for fnmatch (e.g. remove leading ./)
        ...
        comments_for_gh_review_api = analyze_code(actual_files_to_process, pr_details) # This is now parallelized
    ...
    except ValueError as e: logger.error(f"Error in main process: {str(e)}") # Expected
    except Exception as e: logger.error(f"Unexpected error in main process: {str(e)}"); traceback.print_exc() # Unexpected
...
if __name__ == "__main__":
    try:
        ...
        traceback.print_exc()
        try:
            # Attempt to create a minimal error review file for workflow artifact commit
            # This part needs careful error handling itself
            error_pr_details = None
            try: error_pr_details = get_pr_details()
            except: pass # Ignore if can't get PR details in this state
...
**2. GitHub Action Workflow File Modifications (`.github/workflows/your-workflow-name.yml`)**

*   **Add Pip Cache**:
*   **Add `MAX_PARALLEL_FILE_REVIEWS` environment variable**:
*   **Optional: Use `requirements.txt` for dependencies**:
...
jobs:
  gemini-code-review:
    ...
    steps:
      ...
      # Option 1: Create a requirements.txt (Recommended)
      # Create a file .github/workflows/requirements.txt with your dependencies:
      # flake8
      # google-generativeai
      # PyGithub
      # unidiff
      ...
      # Option 2: If not using requirements.txt, cache based on install command (less ideal)
      - name: Cache pip dependencies
        uses: actions/cache@v4
        with:
          path: ~/.cache/pip
          # Key on the workflow file itself or a specific part if dependencies are hardcoded
          key: ${{ runner.os }}-pip-${{ github.workflow }}-${{ hashFiles('**/gemini-pr-review.py') }}
          restore-keys: |
      ...
      - name: 🐍 Python Syntax Check
        # (Your existing syntax check step - no changes needed for this optimization)
        run: |
          echo "Running Python validation on .github/ and other specified .py files..."
          set -e; errors=0; summary=""
          py_files_to_check=$(find .github -type f -name "*.py" 2>/dev/null)
          if [ -z "$py_files_to_check" ]; then echo "No Python files found."; else
            echo "Step 1: Basic syntax check"; for file in $py_files_to_check; do echo "Checking $file"; if ! output=$(python -m py_compile "$file" 2>&1); then errors=$((errors+1)); summary+="$file (py_compile error):\n$output\n\n"; fi; done
            echo "\nStep 2: AST parsing issues"; ast_errors=0; ast_summary=""; for file in $py_files_to_check; do if ! python -c "import ast; ast.parse(open('$file', encoding='utf-8').read())" 2>&1 >/dev/null; then if ! grep -q "$file (py_compile error)" <<< "$summary"; then ast_errors=$((ast_errors+1)); ast_summary+="$file (AST error):\nAST parser failed.\n\n"; fi; fi; done
            if [ "$ast_errors" -gt 0 ]; then summary+=$ast_summary; errors=$((errors + ast_errors)); fi
            echo "\nStep 3: flake8 critical errors"; flake8_errors=0; flake8_summary=""; for file in $py_files_to_check; do output=$(flake8 --select=E9,F821 "$file" 2>&1); if [ -n "$output" ]; then if ! grep -q "$file" <<< "$summary"; then flake8_errors=$((flake8_errors+1)); flake8_summary+="$file (flake8 E9/F821):\n$output\n\n"; fi; fi; done
            if [ "$flake8_errors" -gt 0 ]; then summary+=$flake8_summary; errors=$((errors + flake8_errors)); fi
          fi
          if [ "$errors" -gt 0 ]; then echo -e "\n⚠️ Issues found:\n$summary"; exit 1; else echo "\n✅ No critical issues detected."; fi

      - name: 🔑 Prepare Gemini API Keys
        id: prepare_gemini_keys
        # (Your existing key preparation step - no changes needed for this optimization)
        env:
          ALL_API_KEYS_JSON_SECRET: ${{ secrets.ALL_API_KEYS_JSON }}
        run: |
          echo "Attempting to parse ALL_API_KEYS_JSON_SECRET..."
          if [ -z "$ALL_API_KEYS_JSON_SECRET" ]; then echo "Error: ALL_API_KEYS_JSON_SECRET is empty." >&2; exit 1; fi
          API_KEYS_NEWLINE_SEPARATED=$(jq -e -r '.[]' <<< "$ALL_API_KEYS_JSON_SECRET")
          JQ_EXIT_CODE=$?
          if [ $JQ_EXIT_CODE -ne 0 ]; then echo "Error: jq failed. Validate JSON. JQ Exit Code: $JQ_EXIT_CODE" >&2; exit 1; fi
          mapfile -t API_KEYS_ARRAY <<< "$API_KEYS_NEWLINE_SEPARATED"
          local_api_keys_count=${#API_KEYS_ARRAY[@]}
          echo "Parsed ${local_api_keys_count} keys."
          SELECTED_KEYS=()
          if [ "${local_api_keys_count}" -eq 0 ]; then echo "Error: No API keys found." >&2; exit 1;
          elif [ "${local_api_keys_count}" -eq 1 ]; then SELECTED_KEYS+=("${API_KEYS_ARRAY[0]}"); SELECTED_KEYS+=("");
          else
            mapfile -t SHUFFLED_KEYS < <(printf "%s\n" "${API_KEYS_ARRAY[@]}" | shuf)
            SELECTED_KEYS+=("${SHUFFLED_KEYS[0]}"); SELECTED_KEYS+=("${SHUFFLED_KEYS[1]}");
          fi
      ...
      - name: 🧠 Run code review
        env:
          ...
          GEMINI_API_KEY: ${{ steps.prepare_gemini_keys.outputs.gemini_main_key }}
          GEMINI_FALLBACK_API_KEY: ${{ steps.prepare_gemini_keys.outputs.gemini_fallback_key }}
          GEMINI_MODEL: gemini-1.5-flash-latest # Updated model name from your original, adjust if needed
          ...
          # Add this environment variable for configurable parallelism
          MAX_PARALLEL_FILE_REVIEWS: "3" # Adjust default (e.g., 2-5) based on testing and API limits
          # LAST_RUN_SHA: ${{ ... }} # You might need to persist/retrieve this across workflow runs for incremental diffs
                                   # This is complex and typically requires an action to store/retrieve commit SHAs.
                                   # For now, the Python script defaults to PR base if LAST_RUN_SHA is not set or applicable.
        run: |
          max_attempts=3; attempt=1
          while [ $attempt -le $max_attempts ]; do
            echo "Attempt $attempt of $max_attempts to run code review script"
            # Make sure your script is executable: chmod +x .github/workflows/gemini-pr-review.py
            if python .github/workflows/gemini-pr-review.py; then
              echo "Code review script completed successfully"; break
            else
              exit_code=$?; echo "Code review script failed with exit code $exit_code"
              if [ $attempt -lt $max_attempts ]; then
                sleep_time=$((60 * attempt)); echo "Waiting $sleep_time seconds before retrying..."; sleep $sleep_time
              else
                echo "All attempts for code review script failed. Continuing to commit artifacts."; # Allow commit even on failure
              fi
            fi
            attempt=$((attempt + 1))
          done
      ...
      - name: 💾 Commit review artifacts (review JSON)
        # (Your existing commit step - no changes needed for this optimization)
        run: |
          ...
          if git diff --staged --quiet --exit-code; then echo "No changes to review artifacts."; else
            commit_message="📊 Update review reports" # Default
            if [ -f "reviews/gemini-pr-review.json" ]; then
              review_comments_count=$(jq '.review_comments | length' reviews/gemini-pr-review.json)
              rate_limited=$(jq -r '.metadata.all_keys_rate_limited_during_review' reviews/gemini-pr-review.json) # Use -r for raw string
              if [ "$review_comments_count" -gt 0 ]; then
                commit_message="👁️‍🗨️ Update Gemini PR review ($review_comments_count issues${rate_limited:+, rate limited})"
              else
                commit_message="👁️‍🗨️ Update Gemini PR review (no issues found${rate_limited:+, rate limited})"
              fi
            fi
            git commit -m "$commit_message"
            for i in 1 2 3; do
              git pull --rebase origin ${{ github.event.pull_request.head.ref }} || true
              if git push origin HEAD:${{ github.event.pull_request.head.ref }}; then echo "Push succeeded."; break; fi
              if [ "$i" -eq 3 ]; then echo "Push failed after attempts."; else echo "Push failed, retrying..."; sleep 10; fi
            done
          fi
```
...
*   **`MAX_PARALLEL_FILE_REVIEWS`**: Start with a small number (e.g., 2 or 3). Too many parallel requests can still hit API rate limits quickly, even with the existing rotation and delay logic. Monitor your API usage and adjust this value.
*   **API Rate Limits (`GEMINI_RPM_LIMIT`)**: The Python script's `GEMINI_REQUEST_INTERVAL_SECONDS` is derived from this. Ensure it's set appropriately. The `gemini_api_interaction_lock` will serialize the actual `generate_content` calls, and `enforce_gemini_rate_limits` (with its own lock) will ensure the time delay between these serialized calls.
*   **Testing**: Thoroughly test these changes to ensure stability and that review quality remains high. Pay attention to logs for any concurrency-related issues or increased rate limiting.
*   **`LAST_RUN_SHA` for Incremental Diffs**: Your Python script has logic for `LAST_RUN_SHA`. Persisting this SHA across workflow runs for true incremental diffs on `synchronize` events (other than the first push) requires additional GitHub Actions mechanisms (e.g., using an action to cache/retrieve the SHA, or committing it to a file in a control branch). The current setup defaults to comparing against the PR's base SHA if `LAST_RUN_SHA` isn't effectively managed across distinct workflow executions.

These changes should provide a noticeable speed-up for PRs with multiple files by overlapping I/O and processing tasks, while the locking mechanisms aim to maintain correctness and manage API interactions safely.